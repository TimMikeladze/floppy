...
export async function parseEpubFile(
  ...
  const { onProgress, signal, pageWidth = 800, pageHeight = 1200 } = options;
  ...
  for (let i = 0; i < spine.length; i++) {
    ...
    if (!manifestItem) {
      console.warn(
        `[epub-parser] Spine item ${spineItem.id} not found in manifest`,
      );
      continue;
    }
...
async function renderTextToPages(
  ...
  for (let i = 0; i < lines.length; i += linesPerPage) {
    ...
    const pageBlob = await renderPageToBlob(
      pageLines,
      pageWidth,
      pageHeight,
      padding,
...
function wrapText(text: string, maxWidth: number, fontSize: number): string[] {
  ...
  for (const paragraph of paragraphs) {
    ...
    for (const word of words) {
      const testLine =
        currentLine +
        (currentLine.endsWith("### ") || currentLine === "" ? "" : " ") +
        word;
