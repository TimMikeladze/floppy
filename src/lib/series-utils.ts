...
export function isSameSeries(comic1: Comic, comic2: Comic): boolean {
  ...
  if (comic1.series && comic2.series) {
    return normalizeSeriesName(comic1.series) === normalizeSeriesName(comic2.series)
  }
  ...
  return normalizeSeriesName(parsed1.seriesName) === normalizeSeriesName(parsed2.seriesName)
}
 ...
 */
export function findNextIssue(currentComic: Comic, allComics: Comic[]): Comic | null {
  ...
  const seriesComics = allComics
    ...
    .filter((item): item is { comic: Comic; issue: number } => item.issue !== null)
 ...
 */
export function findPreviousIssue(currentComic: Comic, allComics: Comic[]): Comic | null {
  const currentIssue = getIssueNumber(currentComic)
