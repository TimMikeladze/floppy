import type { CustomRelease, Release, ReleaseOverride } from "./releases-types";
import type {
  Bookmark,
  Comic,
  ComicList,
...
async function initDB(): Promise<IDBDatabase> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      db.onversionchange = () => {
        console.warn(
          "[storage] Database version changed in another context, closing connection",
        );
        db?.close();
        db = null;
      };
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!database.objectStoreNames.contains(COMICS_STORE)) {
        const comicsStore = database.createObjectStore(COMICS_STORE, {
          keyPath: "id",
        });
      ...
      if (!database.objectStoreNames.contains(BOOKMARKS_STORE)) {
        ...
        bookmarksStore.createIndex("pageNumber", "pageNumber", {
          unique: false,
        });
      }
      ...
      if (!database.objectStoreNames.contains(REMOTE_PAGES_STORE)) {
        const remotePagesStore = database.createObjectStore(
          REMOTE_PAGES_STORE,
          { keyPath: "comicId" },
        );
      ...
      if (!database.objectStoreNames.contains(RELEASES_OVERRIDES_STORE)) {
        const overridesStore = database.createObjectStore(
          RELEASES_OVERRIDES_STORE,
          { keyPath: "releaseId" },
        );
      ...
      if (!database.objectStoreNames.contains(CUSTOM_RELEASES_STORE)) {
        const customReleasesStore = database.createObjectStore(
          CUSTOM_RELEASES_STORE,
          { keyPath: "id" },
        );
        customReleasesStore.createIndex("releaseDate", "releaseDate", {
          unique: false,
        });
      ...
      if (!database.objectStoreNames.contains(RELEASES_DELETIONS_STORE)) {
        database.createObjectStore(RELEASES_DELETIONS_STORE, {
          keyPath: "releaseId",
        });
      }
    };
  ...
  return new Promise((resolve, reject) => {
    ...
    listsCursor.onsuccess = (event) => {
      ...
      if (cursor) {
        ...
        list.comicIds = list.comicIds.filter(
 ...
 */
export async function savePagesForComic(
  comicId: string,
  pages: Blob[],
): Promise<void> {
  const database = await initDB();
 ...
 * Used for iOS/Safari where File System Access API is unavailable.
 */
export async function getPagesForComic(
  comicId: string,
): Promise<Blob[] | null> {
  const database = await initDB();
  return new Promise((resolve, reject) => {
...
export async function addComicToList(
  listId: string,
  comicId: string,
): Promise<void> {
  const database = await initDB();
...
export async function removeComicFromList(
  listId: string,
  comicId: string,
): Promise<void> {
  const database = await initDB();
...
export async function getRemotePages(
  comicId: string,
): Promise<RemotePages | null> {
  const database = await initDB();
  return new Promise((resolve, reject) => {
...
export async function getComicStorageInfo(): Promise<ComicStorageInfo[]> {
  ...
  for (const comic of comics) {
    ...
    if (comic.sourceType === "local" && comic.fileHandle) {
      try {
        const permission = await comic.fileHandle.queryPermission({
          mode: "read",
        });
  ...
  for (const comic of comics) {
    ...
    try {
      ...
      if (permission === "granted") {
        ...
        const newPermission = await comic.fileHandle.requestPermission({
          mode: "read",
        });
...
export async function getOfflineStatus(
  ...
  const percentCached =
    totalPageCount > 0
      ? Math.round((cachedPageCount / totalPageCount) * 100)
      : 0;
  const isAvailableOffline =
    cachedPageCount > 0 && cachedPageCount >= totalPageCount;
...
export async function saveComicForOffline(
  ...
  try {
    if (comic.sourceType === "local") {
      ...
      onProgress?.({
        currentPage: 0,
        totalPages: remotePages.pages.length,
        status: "loading",
      });
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      const results = request.result.map(
        (r: { releaseId: string }) => r.releaseId,
      );
      resolve(results);
    };
    request.onerror = () => reject(request.error);
