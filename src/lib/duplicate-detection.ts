...
import { parseComicTitle, normalizeSeriesName } from "./series-utils"
...
export function findDuplicates(
  ...
  for (const comic of existingComics) {
    ...
    if (comic.fileName) {
      ...
      const filenameSimilarity = stringSimilarity(normalizedNewFilename, normalizedExisting)
    ...
    if (comic.fileSize && Math.abs(comic.fileSize - file.size) / file.size < 0.01) {
