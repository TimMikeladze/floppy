      ...
      {activeView === "calendar" && (
        <div className="mx-auto max-w-5xl">
          ...
          <div className="mb-6 flex items-center justify-between">
            <h2 className="text-2xl font-bold text-foreground">{format(currentMonth, "MMMM yyyy")}</h2>
                    ...
                    {dayReleases.length > 0 && (
                      <div className="mt-1 flex flex-wrap gap-1">
                        ...
                        {dayReleases.length > 3 && (
                          <span className="text-[10px] text-muted-foreground">+{dayReleases.length - 3}</span>
                        )}
                      </div>
                    )}
                  </button>
...
function PullListView({
  ...
  const subscribedReleases = useMemo(() => {
    ...
    return releases.filter(
      (release) => pullList.isSubscribedToSeries(release.series) && !pullListIds.has(release.id)
    )
  }, [releases, pullList])
      ...
      {pullListReleases.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xl font-bold text-foreground">Individual Releases ({pullListReleases.length})</h2>
          </div>
          <div
            className="grid gap-3 sm:gap-4 md:gap-5 lg:gap-6"
            style={{
            ...
            return (
              <div key={subscription.seriesName} className="mb-8">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg font-semibold text-foreground">{subscription.seriesName}</h3>
                ...
                {seriesReleases.length > 0 ? (
                  ...
                  <div className="rounded-lg border border-border bg-card p-8 text-center">
                    <p className="text-sm text-muted-foreground">No releases currently available for this series</p>
                  </div>
                )}
              </div>
            )
  ...
  return (
    ...
    >
      ...
      <div className="p-3.5 space-y-2">
        <h3 className="font-semibold text-sm leading-tight line-clamp-2">{release.title}</h3>

        <p className="text-xs text-muted-foreground truncate">{release.publisher}</p>
