...
export function ComicViewer({
  ...
  const clampPosition = (
    ...
    const maxX =
      (rect.width * (currentScale - 1)) / 2 + rect.width * boundaryMultiplier;
    const maxY =
      (rect.height * (currentScale - 1)) / 2 + rect.height * boundaryMultiplier;
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    if (e.touches.length === 2 && settings.layoutMode !== "scrolling") {
      ...
      e.touches.length === 1 &&
      (scale > 1 || settings.layoutMode === "scrolling")
    ) {
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    if (e.touches.length === 2 && settings.layoutMode !== "scrolling") {
      ...
      const newScale = Math.max(
        1,
        Math.min(4, touchStartRef.current.scale * scaleDelta),
      );
      ...
      const deltaX =
        (e.touches[0].clientX - panStartRef.current.touchX) * PAN_SENSITIVITY;
      const deltaY =
        (e.touches[0].clientY - panStartRef.current.touchY) * PAN_SENSITIVITY;
  ...
  const nextPageUrl =
    settings.pageLayout === "double" ? pages[currentPage + 1] : null;
            ...
            <img
              src={
                (settings.readingDirection === "rtl"
                  ? nextPageUrl
                  : currentPageUrl) || "/placeholder.svg"
              }
