...
import { BookOpen, CheckCircle2, Heart, Library } from "lucide-react";
...
import {
  getStoredLibraryPreferences,
  saveLibraryPreferences,
} from "@/hooks/use-library-preferences";
import { naturalCollator } from "@/lib/sort-utils";
import { deleteComic, getAllComics, getAllLists } from "@/lib/storage";
...
export function HomePageClient({
  ...
  const [sortBy, setSortBy] = useQueryState(
    "sort",
    parseAsStringLiteral(sortTypes).withDefault("recent"),
  );
  ...
  const [selectedListId, setSelectedListId] = useQueryState(
    "list",
    parseAsString,
  );
    ...
    if (selectedListId) {
      ...
      if (selectedList) {
        filtered = filtered.filter((comic) =>
          selectedList.comicIds.includes(comic.id),
        );
      }
    }
            ...
            <div className="flex gap-2 px-3 py-3 sm:px-4 md:px-6 mx-auto w-full max-w-screen-2xl">
              ...
              {lists.map((list) => {
                const count = comics.filter((comic) =>
                  list.comicIds.includes(comic.id),
                ).length;
                return (
                  <button
                    type="button"
